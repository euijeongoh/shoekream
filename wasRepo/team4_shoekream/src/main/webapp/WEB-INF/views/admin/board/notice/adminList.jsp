					...
					<div class="pagination">
						<div class="pagination_box">
							<div class="prev_btn">
								...
								<a href="/shoekream/admin/notice/search?pno=1&search=<%=x %>">《 </a>
								<% } %>
								<% if(pvo.getStartPage() != 1) { %>
								<a href="/shoekream/admin/notice/search?pno=<%= pvo.getStartPage() - 1 %>&search=<%= x %>">〈 </a>
								<% } %>
							</div>
							...
							<% for(int i = pvo.getStartPage(); i <= pvo.getEndPage(); i++) { %>
								<% if(i == pvo.getCurrentPage()) { %>
									<a href="/shoekream/admin/notice/search?pno=<%= i %>&search=<%= x %>"><%= i %></a>
								<% }else {%>
									<a href="/shoekream/admin/notice/search?pno=<%= i %>&search=<%= x %>"><%= i %></a>
								<% } %>
							<% } %>
							...
							<div class="next_btn_box">
								...
								<a href="/shoekream/admin/notice/search?pno=<%= pvo.getEndPage() + 1 %>&search=<%= x %>"> 〉</a>
								<% } %>
								<% if(pvo.getEndPage() != pvo.getMaxPage()){ %>
								 <a href="/shoekream/admin/notice/search?pno=<%= pvo.getMaxPage() %>&search=<%= x %>"> 》</a>
								 <% } %>
